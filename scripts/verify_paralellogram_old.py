...
#plot_parallelogram_coloring(starts_reshaped, grid_coloring, parallelogram[0], parallelogram[1], gridsize, f"fixed_colorings/{run_id}_{gridsize}_initial.png")
...
# Conflict Analysis
# ----------------------
def analyze_conflicts(results, centers, grid_coloring, mask):
    five_non_fixable = []
    five_fixable = []
    ...
    for center in five_fixable:
        ...
        for di, dj in mask:
            ...
            if mc != last_color_idx:
                used_colors.add(mc)
        unused = [i for i in range(last_color_idx) if i not in used_colors]
        if unused:
...
stats = analyze_conflicts(results, centers, grid_coloring, base_mask)
...
total_remaining = len(stats["conflict_non_fixable"]) + len(stats["five_non_fixable"]) + len(stats["conflict_fixable"]) + len(stats["five_fixable"])
print(f"Iteration {count}: Remaining conflicts: {total_remaining} ({total_remaining / (gridsize**2) * 100:.8f}%)")


while len(stats["conflict_fixable"]) + len(stats["conflict_non_fixable"]) + len(stats["five_fixable"]) > 0:

    #plot_parallelogram_coloring(starts_reshaped, next_iteration, parallelogram[0], parallelogram[1], gridsize, f"fixed_colorings/{run_id}_{gridsize}_iteration_{count}.png")
    count += 1
    ...
    stats = analyze_conflicts(results, centers, next_iteration, base_mask)
    total_remaining = len(stats["conflict_non_fixable"]) + len(stats["five_non_fixable"]) + len(stats["conflict_fixable"]) + len(stats["five_fixable"])
    print(f"Iteration {count}: Remaining conflicts: {total_remaining} ({total_remaining / (gridsize**2) * 100:.8f}%)")
...
save_fixed_fraction(run_id, gridsize, fraction_fixed, csv_filename='FINAL_VERIFIED_PARALLELOGRAMS.csv')
